            ...
            for product_id in product_template["product_variant_ids"]:
                ...
                for subcontractor in subcontractors:
                    ...
                    if (
                        ...
                        # Handle scrap_rate calculation for '<load quantity='
                        bom_line_quantities = []                  
                        ...
                        for j in self.generator.getData(
                            ...
                            # Handle scrap_rate calculation
                            bom_line_quantities.append(self.convert_qty_uom(
                                j["product_qty"],
                                j["product_uom_id"],
                                j["product_id"][0],
                            ))
                        ...
                        # Handle scrap_rate
                        # print("mschulz BOM: "+str(bom_line_quantities) + str(producedQty))
                        # scrap_rate_bom = (max(bom_line_quantities) - producedQty) / producedQty if bom_line_quantities and producedQty < max(bom_line_quantities) else 1
                        scrap_rate_bom = max(bom_line_quantities) / producedQty if bom_line_quantities and producedQty < max(bom_line_quantities) else 1
                        print("mschulz: wo: "+str(max(bom_line_quantities) if bom_line_quantities and producedQty < max(bom_line_quantities) else 1)+"| "+str(producedQty)+"| scrap: "+str(scrap_rate_bom))
            ...
            if not wo_list:
                ...
                for wo in wo_list:
                    ...
                    for mv in mv_list:
                        ...
                        # Handle scrap_rate
                        component_quantities.append(self.convert_qty_uom(
                            mv["product_qty"],
                            mv["product_uom"],
                            self.product_product[mv["product_id"][0]]["template"],
                        ))
                    ...
                    # Handle scrap_rate, qty is MO qty
                    # scrap_rate_wo = (max(component_quantities) - qty) / qty if component_quantities and qty < max(component_quantities) else 1
                    scrap_rate_wo = max(component_quantities) / qty if component_quantities and qty < max(component_quantities) else 1
                    print("mschulz: wo: "+str(max(component_quantities) if component_quantities and qty < max(component_quantities) else 1)+"| "+str(qty)+"| scrap: "+str(scrap_rate_wo))
                    # print("mschulz: "+str(component_quantities))
